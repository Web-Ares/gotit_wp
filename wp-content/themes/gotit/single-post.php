...
<div class="site__content">
  ...
  <h2 class="site__main-title site__main-title_inner">
    <?=  (!$wordBack)? get_the_title(40) : $wordBack; ?>
  </h2>

  <!-- blog -->
  <div class="blog">

    <h1 class="site__title site__title_3 site__content-title"><?= $title ?></h1>

    <!-- blog__content -->
    <div class="blog__content">
    <?php  $image = get_field('main_image_for_prview_and_top_of_the_post');
    if($image){
      $imageID = $image['ID'];
      ...
      <?php if($image): ?>
      <!-- blog__post-pic -->
      <div class="blog__content-pic">
        <img src="<?= $image['url'] ;?>"  title="<?= $image['description'] ?>" alt="<?= $alt ;?>">
      </div>
      <!-- /blog__post-pic -->
      <?php endif; ?>
      <h2 class="blog__post-title"><?php the_field('header_under_the_main_image') ?></h2>
    ...
    <?php if( have_rows('content_block') ):
      ...
      while ( have_rows('content_block') ) : the_row();
        ...
        <?php elseif($type =='full'): ?>
          ...
          <!-- blog__wide-pic -->
          <div class="blog__wide-pic">
            <?php  $image = get_sub_field('full_width_image');
            if($image){
              $imageID = $image['ID'];
              $alt = get_post_meta($imageID , '_wp_attachment_image_alt', true);
            } ?>
            <img src="<?= $image['url'] ?>"  alt="<?= $alt ?>" title="<?= $image['description'] ?>">
          </div>
          <!-- /blog__wide-pic -->
    ...
    <div class="social-share">
      ...
      <a target="_blank" href="<?= $links['facebook'] ?>"class="social-share__item">
      ...
      <a target="_blank" href="<?= $links['twitter'] ?>" class="social-share__item">
      ...
      <a target="_blank" href="<?= $links['google'] ?>" class="social-share__item">
        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" x="0px" y="0px" width="512px" height="512px" viewBox="0 0 475.092 475.092" style="enable-background:new 0 0 475.092 475.092;" xml:space="preserve">
                            <g>
                              <g>
                                ...
                                <polygon points="401.998,73.089 401.998,0 365.449,0 365.449,73.089 292.358,73.089 292.358,109.636 365.449,109.636     365.449,182.725 401.998,182.725 401.998,109.636 475.081,109.636 475.081,73.089   " fill="#FFFFFF"/>
                              </g>
                            </g>
                        </svg>
      </a>
      <a target="_blank" href="<?= $links['in'] ?>" class="social-share__item">
    ...
    <div class="blog__home">
      <a href="<?= $blogHome ?>" class="btn">BLOG HOME</a>
    </div>
    <!-- /blog__home -->
